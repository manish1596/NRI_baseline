...
class SimulationDecoder(nn.Module):
    ...
    def __init__(self, loc_max, loc_min, vel_max, vel_min, suffix, gpu_number=0):
    ...
    def set_diag_to_one(self, x):
        ...
        if x.is_cuda:
            dev = "cuda:" + str(self.gpu_number)
            device = torch.device(dev)
            mask, inverse_mask = mask.to(device), inverse_mask.to(device)
    ...
    def forward(self, inputs, relations, rel_rec, rel_send, pred_steps=1):
        ...
        if inputs.is_cuda:
            ...
            offdiag_indices = offdiag_indices.to(device)
...
class MLPDecoder(nn.Module):
    ...
    def __init__(self, n_in_node, edge_types, msg_hid, msg_out, n_hid,
                 do_prob=0., skip_first=False, gpu_number=0):
