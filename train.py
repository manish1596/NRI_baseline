...
parser.add_argument('--gpu_number', type=int, default=0, help='The GPU to be used while training')
...
def train(epoch, best_val_loss):
    ...
    for batch_idx, (data, relations) in enumerate(train_loader):
        ...
        print("relations.shape = "+ str(relations.shape))

        optimizer.zero_grad()

        logits = encoder(data, rel_rec, rel_send)
        #print("logits.shape=" + str(logits.shape))
        edges = gumbel_softmax(logits, tau=args.temp, hard=args.hard, gpu_number=args.gpu_number)
    ...
    for batch_idx, (data, relations) in enumerate(valid_loader):
        ...
        logits = encoder(data, rel_rec, rel_send)
        edges = gumbel_softmax(logits, tau=args.temp, hard=True, gpu_number=args.gpu_number)
        prob = my_softmax(logits, -1)
...
def test():
    ...
    for batch_idx, (data, relations) in enumerate(test_loader):
        ...
        if args.decoder == 'rnn':
            output = decoder(data_decoder, edges, rel_rec, rel_send, 100,
                             burn_in=True,
                             # run with auto-regressive
                             burn_in_steps=0)
                             #burn_in_steps=args.timesteps - args.prediction_steps)
